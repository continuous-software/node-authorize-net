...
function AuthorizeNetGateway (options) {
  ...
  assert.ok(options.API_LOGIN_ID, 'API_LOGIN_ID must be provided');
  assert.ok(options.TRANSACTION_KEY, 'TRANSACTION_KEY must be provided');
    ...
    return toJson(body)
      .then(function (result) {
        ...
        if (json.ErrorResponse) {
          throw new GatewayError(json.ErrorResponse.messages[0].message[0].text[0], json.ErrorResponse);
        }
...
AuthorizeNetGateway.prototype.sendTransactionRequest = function sendTransactionRequest (body, transactionCb) {
  ...
  return P.resolve()
    ...
    .spread(createJsonCallback(function (json) {
      if (json.createTransactionResponse) {
        ...
        }  else if(json.createTransactionResponse.transactionResponse[0] && json.createTransactionResponse.transactionResponse[0].errors && json.createTransactionResponse.transactionResponse[0].errors[0]){
          throw new GatewayError(json.createTransactionResponse.transactionResponse[0].errors[0].error[0].errorText[0], json.createTransactionResponse);
        }
...
AuthorizeNetGateway.prototype.getSettledBatchList = function getSettledTransactionsList (from, to) {
  ...
  return P.resolve()
    .then(function () {
      var obj = setRequest(service, 'getSettledBatchListRequest', {
    ...
    .spread(createJsonCallback(function (json) {
      ...
      if (json.getSettledBatchListResponse) {
        ...
        if (json.getSettledBatchListResponse.messages[0].resultCode[0] === 'Error') {

          errors = json.getSettledBatchListResponse.messages[0].message[0];
          errors = errors.length > 1 ? errors : [errors];

          throw new GatewayError(errors.reduce(function (previous, current) {
            previous += '- ' + current.text;
            return previous;
          }, ''), json.getSettledBatchListResponse);
        }
...
AuthorizeNetGateway.prototype.getTransactionList = function getTransactionList (batchId) {
  ...
  return P.resolve()
    .then(function () {
      return setRequest(service, 'getTransactionListRequest', {
        key: 'batchId',
        value: batchId
      });
    })
...
AuthorizeNetGateway.prototype.chargeCustomer = function (order, prospect, other) {
  ...
  return this.getCustomerProfile(prospect.profileId)
    .then(function (res) {
      ...
      var customerPaymentProfileId = other.customerPaymentProfileId;
      ...
      var body = {
        ...
        profile: {
          ...
          paymentProfile: {
            paymentProfileId: customerPaymentProfileId ? customerPaymentProfileId : paymentProfile[0][0].customerPaymentProfileId[0]
          }
        }
      };
